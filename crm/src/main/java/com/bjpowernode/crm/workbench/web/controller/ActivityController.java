...
public class ActivityController {
    ...
    public void fileDownload(HttpServletResponse response) throws IOException {
        ...
        // 读取excel文件（InputSteam），输出到浏览器（OutputSteam）
    ...
    public void exportAllActivities(HttpServletResponse response) throws IOException {
        ...
        // 根据wb对象生成excel文件
        /*OutputStream os = new FileOutputStream("D:\\Intellij IDEA\\crm\\serverDir\\activityList.xls");
        wb.write(os);*/
        // 关闭资源
        /*os.close();
        wb.close();*/
        ...
        // 把生成的excel文件下载到客户端
        // 文件下载要使用同步请求，因为文件下载的时候，需要设置响应头的信息，告诉浏览器不要打开在线打开excel文档
        response.setContentType("application/octet-stream;charset=UTF-8");
        response.addHeader("Content-Disposition","attachment;filename=activityList.xls");
        OutputStream out = response.getOutputStream();
        /*InputStream is = new FileInputStream("D:\\Intellij IDEA\\crm\\serverDir\\activityList.xls");
        byte[] buff = new byte[256];
        int len = 0;
        while ((len = is.read(buff)) != -1){
            out.write(buff,0,len);
    ...
    @RequestMapping("/workbench/activity/exportCheckedActivitiesByIds.do")
    public void exportCheckedActivitiesByIds(String[] id, HttpServletResponse response) throws IOException {
        // 调用service层，查询所有的市场活动
        List<Activity> activityList = activityService.queryCheckedActivitiesByIds(id);
        // 创建excel文件，并且把activityList写入到excel文件中
        HSSFWorkbook wb = new HSSFWorkbook();
        HSSFSheet sheet = wb.createSheet("市场活动列表");
        HSSFRow row = sheet.createRow(0);
    ...
    public @ResponseBody Object fileUpload(String userName, MultipartFile myFile) throws IOException {
        ...
        File file = new File("D:\\Intellij IDEA\\crm\\serverDir\\" + originalFilename); // 路径必须手动创建好，文件如果不存在，会自动创建
    ...
    public @ResponseBody Object importActivity(MultipartFile activityFile, HttpSession session){
        ...
        try {
            ...
            // InputStream is = new FileInputStream("D:\\Intellij IDEA\\crm\\serverDir\\" + originalFilename);
